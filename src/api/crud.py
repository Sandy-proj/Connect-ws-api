...
from ..core_logic import init_board
...
async def get_game_by_id(id: PyObjectId) -> Game | None:
    client = MongoDBClient()
    game_data = await client.get(Game, id)
...
async def list_games_from_db() -> list[Game]:
    client = MongoDBClient()
    return await client.list(Game)


async def delete_game_by_id(id: PyObjectId) -> int:
    return 1


async def delete_games_from_db() -> int:
    client = MongoDBClient()
    result = await client.delete_many(Game)
...
async def update_game(game_id: PyObjectId, game_data: dict[str, any]) -> Game | None:
    client = MongoDBClient()
    await client.update_one(Game, game_id, game_data)
