import logging

from fastapi import APIRouter, HTTPException, WebSocket, status

from .crud import (
    delete_games_from_db,
    get_game_by_id,
    join_new_game,
    list_games_from_db,
    start_new_game,
    update_game,
)
from .fields import PyObjectId
from .models import Game, MoveInput, StartGame, get_model_safe
from .shortcuts import make_move
from .validators import validate
from .websockets import connection_manager

logger = logging.getLogger(__name__)


router = APIRouter(prefix="/games", tags=["Games"])


@router.post("/")
async def start_game(player_data: StartGame) -> Game:
    print(type(player_data))
    game = await start_new_game(player_data.player)
    if game is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Game cannot be started now, please try again later.",
        )
    return game
    # data = {"player1":player_data.player, "player2":player_data.player}
    # client = MongoDBClient()
    # insert_result = await client.insert(Game, data)
    # game_data = await client.get(Game,insert_result.inserted_id)
    # result = Game(**game_data)
    # return result


@router.get("/")
async def list_games() -> list[Game]:
    return await list_games_from_db()


@router.get("/{game_id}/")
async def get_game(game_id: PyObjectId) -> Game:
    game = await get_game_by_id(game_id)
    if game is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Game not found"
        )
    return game


@router.delete("/")
async def delete_game() -> dict[str, int]:
    result = await delete_games_from_db()
    return {"deleted_games_count": result}


@router.post("/{game_id}/join")
async def join_game(game_id: PyObjectId, player_data: StartGame) -> Game:
    game = await get_game_by_id(game_id)
    if game is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Game not found"
        )

    if game.player2 is not None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Game already started"
        )

    updated_game = await join_new_game(game, player_data.player)
    if updated_game is None:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Game cannot be joined right now. Please try again later.",
        )

    await connection_manager.broadcast_game(updated_game)

    return updated_game


@router.websocket("/ws/{game_id}/")
async def websocket_game_endpoint(websocket: WebSocket, game_id: PyObjectId) -> None:
    print("websocket")
    await connection_manager.connect(websocket, game_id)

    if True:
        while True:
            move_data = await websocket.receive_json()

            game = await get_game_by_id(game_id)

            move = get_model_safe(MoveInput, move_data)

            # Validation for move.
            error_message = validate(game, move)
            if error_message:
                logger.warning(error_message)
                print(error_message)
                continue

            make_move(game, move.col)

            updated_game = await update_game(game.id, game.model_dump())  # type: ignore[union-attr]
            print("broadcasting")
            await connection_manager.broadcast_game(updated_game)  # type: ignore[arg-type]
